 ...
 * v2 - corrigido null fingerprint + retorna donation_id
 */
...
// ✅ CORRIGIDO: substr em null causava erro fatal e resposta vazia
logDebug('Nova solicitação de pagamento', [
    'amount'      => $amount,
    'email'       => $email,
    'fingerprint' => $fingerprint ? substr($fingerprint, 0, 20) . '...' : 'null',
]);
...
try {
    $donationId = generateId('don'); // ✅ gera ID aqui para usar no retorno
    ...
    $preference = [
        ...
        'external_reference'   => $donationId, // ✅ usa o ID gerado
        'metadata' => [
    ...
    // ✅ Retorna donation_id para o frontend salvar no sessionStorage
    jsonResponse(true, [
        'preference_id'      => $result['id'],
        'init_point'         => $result['init_point'],
    ...
    logDebug('Exceção ao criar pagamento', ['error' => $e->getMessage()]);
